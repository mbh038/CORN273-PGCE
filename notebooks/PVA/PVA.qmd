...
[InsightMaker](https://insightmaker.com) is a web-based visual programming language for modelling the dynamics of inter-connected systems.
...
A \[Stock\] of stuff increases over time via what \[Flows\] in. Imagine our stock represents *Amur Tiger*

*Amur Tiger Births* would then be the input \[Flow\] to the population of *Amur Tiger* over time.

A \[Stock\] of stuff decreases over time via what \[Flows\] out. For example, *Amur Tiger Death*s could represent the \[Flow\] out of the population of *Amur Tiger*.

```{r}
#| label: fig-amur-tiger1
#| fig-cap: :"The simplest Amur Tiger population model"
knitr::include_graphics(here("figures","amur-tiger1.png"))
```
...
4.  Now click the blue Simulate button. You have run your first population simulation!
...
```{r}
#| label: fig-amur-tiger2
#| fig-cap: :"A better Amur tiger population model that incorporates exponential change"
knitr::include_graphics(here("figures","amur-tiger2.png"))
```
...
4.  Make sure the initial number of Amur tiger is set to 50, the birth rate is set to 0.5, and the death rate is set to 0.4. Now click \[Run Simulation\]. How would you describe the results?
...
birth_rate <- 0.5
death_rate <- 0.5
years <- seq(1:year_max)

stock = rep(N0,year_max)
for (i in 2:length(stock)){
  # stock[i] = stock[i-1] + rpois(1,birth_rate * stock[i-1]) - rbinom(1, stock[i-1], death_rate ) # for a fluctuating population
  stock[i] = stock[i-1] + 0.022 * stock[i-1] # without fluctuations
}

df <- data.frame(years,stock)

# df |>
#   ggplot(aes(x = years, y = stock)) +
#   geom_line(linewidth = 1.0, colour = "darkgreen") +
#   labs(x = "Time (Years)", y = "Amur tiger population") +
...
```{r}
#| label: fig-fluct2
#| fig-cap: "A realistic pattern of population change for a wild animal species"
knitr::include_graphics(here("figures","fluct2.png"))
```
...
```{r}
#| label: fig-nofluct2
#| fig-cap: "An unrealistic pattern of population change for a wild animal species. In reality, the population would most likely fluctuate."
knitr::include_graphics(here("figures","nofluct2.png"))
```
